(function () {
    ...
    function StudiosView(object) {
        ...
        comp.create = function () {
            ...
            network.silent(Lampa.TMDB.api(object.url + '?' + params.join('&')), function (json) { _this.build(json); }, _this.empty.bind(_this));
        };
    ...
    function injectUI() {
        ...
        // ГЛАВНАЯ СТРАНИЦА
            ...
            MENU_ORDER.forEach(function (sid) { $('.menu [data-sid="' + sid + '"]').remove(); });

            var active = Lampa.Activity.active();
            if (active && active.component === 'main') {
                // Ищем ЛЮБОЙ активный скролл-контейнер
                var container = $('.activity.active').find('.items, .scroll__content, .main__content').first();
                
                // Если стандартные классы не найдены, используем поиск по структуре (для CUB)
                if (!container.length) {
                    container = $('.activity.active').find('div > div:has(.card)').parent();
                }

                if (container.length && !container.find('.studios-home-row').length) {
                    var items = [];
                    MENU_ORDER.forEach(function (sid) { 
                        var c = SERVICE_CONFIGS[sid]; 
                    ...
                    // Вставка после первого блока (например, после "Сейчас смотрят")
                    var firstRow = container.children().first();
    ...
    if (window.Lampa) startPlugin();
        ...
        var wait = setInterval(function() { if (window.Lampa) { clearInterval(wait); startPlugin(); } }, 200);
    }
})();
