(function () {
    ...
    // 1. НАПОЛНЕНИЕ (ТВОИ ОРИГИНАЛЬНЫЕ КОНФИГИ БЕЗ ИЗМЕНЕНИЙ)
    var ICONS = {
    ...
    function init() {
        ...
        // Интеграция через ContentRows с исправленным рендером SVG
                ...
                return function(call) {
                    ...
                    items.forEach(item=>{
                        item.params = {
                            ...
                            // ИСПРАВЛЕНИЕ: Прямая инъекция SVG в HTML-контейнер карточки
                            createInstance: function(item){ 
                                var card = Lampa.Maker.make('Card', item, (module)=>module.only('Card','Style','Callback'));
                                card.render().find('.card__img').html(item.img); // Вставляем SVG напрямую
                                return card;
                            },
                            emit: {
                                onlyEnter: function(){
        ...
        $('body').append('<style>.studios_row .card{width:11em!important; height:6em!important;}.studios_row .card__img svg{width: 70%; height: 70%; margin: 15% auto; display: block;}.studios_row .card__ico{display:flex; align-items:center; justify-content:center; height:100%; padding:15px; background: rgba(255,255,255,0.05); border-radius: 10px;}.studios_row .card.focus .card__ico{background: rgba(255,255,255,0.15); border: 2px solid #fff;}</style>');
    }
