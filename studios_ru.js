(function () {
    ...
    // 1. ИКОНКИ И КОНФИГИ (ВОЗВРАЩЕНО К РАБОЧЕМУ СОСТОЯНИЮ)
    var ICONS = {
    ...
    // 3. УПРАВЛЕНИЕ ИНТЕРФЕЙСОМ (ТОЛЬКО ЗДЕСЬ ИЗМЕНЕНИЯ ДЛЯ ГЛАВНОЙ)
    function injectUI() {
        var mode = Lampa.Storage.get('studios_display_type', 'menu');
        
        // ВОЗВРАЩЕНО РАБОЧЕЕ МЕНЮ (КАК БЫЛО В САМОМ НАЧАЛЕ)
        if (mode === 'menu') {
            ...
            if (menu.length) {
                MENU_ORDER.forEach(function (sid) {
                    ...
                    btn.on('hover:enter', function () { 
                         Lampa.Activity.push({ title: c.title, component: 'studios_main', service_id: sid }); 
                    });
                    menu.append(btn);
                });
            }
        ...
        // ТОЛЬКО ДЛЯ ГЛАВНОЙ
            ...
            if (active.length && !active.find('.studios-home-row').length) {
                // Поиск по точному заголовку "Сейчас смотрят"
