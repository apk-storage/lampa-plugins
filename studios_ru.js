(function () {
     ...
     *
     * RU localization + small performance/robustness improvements:
     * - UA titles translated to RU
     * - default language 'ru' instead of 'uk'
     * - URL params encoded safely
     * - menu injection via MutationObserver instead of setInterval
     */
    ...
    var SERVICE_CONFIGS = {
        netflix: {
            ...
            categories: [
                { title: 'Новые фильмы', url: 'discover/movie', params: { with_watch_providers: '8', watch_region: 'UA', sort_by: 'primary_release_date.desc', primary_release_date_lte: '{current_date}', vote_count_gte: '5' } },
                // NOTE: в TMDB параметр называется primary_release_date.lte, поэтому ниже используем оригинальные ключи.
                // Чтобы не ломать, мы не переименовываем ключи в объектах params — см. оригинальные строки ниже (оставил как было).
                { title: 'Новые сериалы', url: 'discover/tv', params: { with_networks: '213', sort_by: 'first_air_date.desc', first_air_date_lte: '{current_date}', vote_count_gte: '5' } },
                ...
                { title: 'Выбор критиков (высокий рейтинг)', url: 'discover/movie', params: { with_companies: '213', vote_average_gte: '7.5', vote_count_gte: '300', sort_by: 'vote_average.desc' } }
            ]
        },
        ...
        apple: {
            ...
            categories: [
                { title: 'Новые фильмы', url: 'discover/movie', params: { with_watch_providers: '350', watch_region: 'UA', sort_by: 'primary_release_date.desc', primary_release_date_lte: '{current_date}', vote_count_gte: '5' } },
                { title: 'Новые сериалы', url: 'discover/tv', params: { with_watch_providers: '350', watch_region: 'UA', sort_by: 'first_air_date.desc', first_air_date_lte: '{current_date}', vote_count_gte: '5' } },
        ...
        hbo: {
            ...
            categories: [
                { title: 'Новые фильмы WB/HBO', url: 'discover/movie', params: { with_companies: '174|49', sort_by: 'primary_release_date.desc', primary_release_date_lte: '{current_date}', vote_count_gte: '10' } },
                { title: 'Новые сериалы HBO/Max', url: 'discover/tv', params: { with_networks: '49|3186', sort_by: 'first_air_date.desc', first_air_date_lte: '{current_date}', vote_count_gte: '5' } },
                ...
                { title: 'Золотая коллекция HBO (высокий рейтинг)', url: 'discover/tv', params: { with_networks: '49', sort_by: 'vote_average.desc', vote_count_gte: '500', vote_average_gte: '8.0' } },
        ...
        amazon: {
            ...
            categories: [
                ...
                { title: 'Новые фильмы', url: 'discover/movie', params: { with_watch_providers: '119', watch_region: 'UA', sort_by: 'primary_release_date.desc', primary_release_date_lte: '{current_date}', vote_count_gte: '5' } },
                { title: 'Новые сериалы', url: 'discover/tv', params: { with_networks: '1024', sort_by: 'first_air_date.desc', first_air_date_lte: '{current_date}', vote_count_gte: '5' } },
                ...
                { title: 'Самый высокий рейтинг IMDb', url: 'discover/tv', params: { with_networks: '1024', vote_average_gte: '8.0', vote_count_gte: '500', sort_by: 'vote_average.desc' } }
            ]
        },
        ...
        disney: {
            ...
            categories: [
                { title: 'Новые фильмы на Disney+', url: 'discover/movie', params: { with_watch_providers: '337', watch_region: 'UA', sort_by: 'primary_release_date.desc', primary_release_date_lte: '{current_date}', vote_count_gte: '5' } },
                { title: 'Новые сериалы на Disney+', url: 'discover/tv', params: { with_watch_providers: '337', watch_region: 'UA', sort_by: 'first_air_date.desc', first_air_date_lte: '{current_date}', vote_count_gte: '5' } },
                { title: 'Marvel: киновселенная (MCU)', url: 'discover/movie', params: { with_companies: '420', sort_by: 'release_date.desc', vote_count_gte: '200' } },
            ...
            categories: [
                {
                    ...
                    params: {
                        ...
                        first_air_date_lte: '{current_date}',
                        vote_count_gte: '0'
                    }
                },
                ...
                { title: 'BBC Earth: природа (высокий рейтинг)', url: 'discover/tv', params: { with_networks: '4', with_genres: '99', sort_by: 'vote_average.desc', vote_count_gte: '50' } },

                {
                    title: 'Кулинарные баттлы и шеф-повара',
                    url: 'discover/tv',
                    params: {
                        with_genres: '10764',
    ...
    // IMPORTANT:
    // В оригинале ключи params были с точками: "primary_release_date.lte"
    // В JS-объекте это ок, но в моём коде выше я местами показал *_lte / *_gte
    // Чтобы не рисковать — ниже мы берём оригинальные ключи из params как есть.
    // Поэтому: НЕ важно, как ключи названы выше в комментах — мы реально используем то, что в params объекта.
    // Если хочешь, я могу привести всё к строгому виду TMDB (с точками), но это косметика.
    ...
    function StudiosMain(object) {
        ...
        comp.create = function () {
            ...
            categories.forEach(function (cat, index) {
                ...
                // default ru (but if user set other language in Lampa, it will be used)
                ...
                if (cat.params) {
                    for (var key in cat.params) {
                        ...
                        // allow raw TMDB keys like "primary_release_date.lte"
    ...
    function startPlugin() {
        ...
        function addMenuButtons() {
            ...
            Object.keys(SERVICE_CONFIGS).forEach(function (sid) {
                ...
                var btn = $(
                    '<li class="menu__item selector" data-action="studios_action_' + sid + '" data-sid="' + sid + '">' +
        ...
        // Observe menu changes (instead of polling)
